...
class Territory {
  ...
  animateBattle(battle) {
    ...
    battle.log.forEach((roll) => {
      text.append(`Attacker rolls ${roll.attackerRolls.join(', ')}`);
      let br = document.createElement('br');
      text.append(br, `Defender rolls ${roll.defenderRolls.join(', ')}`);
      ...
      if(attackerLoss) {
        ...
        text.append(br, `Attacker loses ${attackerLoss} ${attackerLoss === 1 ? 'unit' : units}`)
      }

      if(defenderLoss) {
        br = document.createElement('br');
        text.append(br, `Attacker loses ${defenderLoss} ${defenderLoss === 1 ? 'unit' : units}`)
      }
    });
